...
## 4. Blender headless（可选 · 批量/QA 段，无 MCP）

- **定位**：非关键路径的可选批处理段（详见 `README.md`「可选 · Blender headless 批量/QA 阶段」）；触发条件：单批 > 10 件 / 坏网格修复 / LOD 与碰撞网格 / 面数校验报告。
- **安装**：本机装 Blender 4.x（LTS 即可），无需任何插件或 API key；`blender` 命令加入 PATH。
- **调用**：`blender --background --python qa_batch.py -- <输入目录> <输出目录> <面数预算表.json>`；脚本职责四件——批量减面（decimate 到预算）、面数自动校验（超标拒收记日志）、网格修复（法线/流形/原点/Y-up）、批量导出（`loc_{地点}_{类别}_{名称}.glb`，单件 < 30 KB）。
- **面数预算表**：从 spec 建材清单导出为 JSON（资产名 → 预算三角数），作为脚本唯一输入源——预算数字一律回源 spec，不在脚本里自创。
- **边界**：Blender 段不做雕刻/重拓扑/UV/烘焙/绑定（本项目资产画像刻意避开）；色值校验不在 Blender 做（④ 步在 Godot 端用顶点色/材质球覆写）。
